...
// AES-256-GCM envelope used to wrap secrets at rest — currently just the
// Google refresh token, but written generic so the same helper can be
// reused for future per-user secrets (Slack tokens, signed URLs we want
// to durably stash, etc.).
//
// Wire format: `v1:<iv-b64url>:<ciphertext-b64url>:<auth-tag-b64url>`
// The version prefix lets us rotate the algorithm later without a data
// migration — old rows decrypt with the v1 path, new rows write v2.
...
// Convenience for tests + admin CLIs — generates a fresh key in the
// expected base64 form. Not used at runtime.
